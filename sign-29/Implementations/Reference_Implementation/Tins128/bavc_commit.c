                 ...
                 commitment coms[TAU][N], hash_t H_com, node seeds[TAU][N])
{
    ...
    for (int e = 0; e < TAU; e++)
    {
        for (int i = 0; i < N; i++)
        {
            ...
            sm3hash(256, msg, (SALT_SIZE + NODE_SIZE + 5) * 8, coms[e][i]);
        }
    }
    ...
    sm3hash(256, msg1, N_LEAVES * COMMIT_SIZE * 8 + 8, H_com);
}
                ...
                ff12b delta[TAU], hash_t h_sh)
{
    ...
    sm3hash(256, msg, (1 + SALT_SIZE + sizeof(H_com))*8 + (N_TUPLE-2)*2*TAU, h_sh);
